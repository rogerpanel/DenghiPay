...
### Phase 14 — Sender product surface (added 2026-09-08, out of plan order)

A gap analysis against a mature diaspora remittance app (`docs/FEATURE_GAP_LEMFI.md`) produced a list; this phase is the part of it that sits inside our existing licensing posture. Nothing here writes financial state, and no ledger code was touched.

- **14.1 In-app notifications.** A per-user store written beside the existing outbox at the same call site. An update that reaches a sender's inbox and not their app is a support ticket; one that reaches neither is how somebody concludes their money is lost. Links are validated as in-app paths — a notification that could carry an external URL would be a phishing primitive we shipped ourselves.
- **14.2 Receipts.** Proof of payment for a terminal transfer, printable from the browser rather than through a PDF service. Three properties are load-bearing: it is refused for a transfer still in flight, because people forward these to landlords and universities and a promise reads identically to a fact; it names the payer, fetched from the residency partition at the moment it is produced and never copied into the neutral tier; and issuing one is audited, because it is a read of personal data joined to a payment.
- **14.3 Send again.** Prefill from a completed transfer. Prefill _only_ — the send flow re-quotes at today's rate, re-screens and re-checks limits exactly as it does for a transfer typed from scratch.
- **14.4 Standing instructions.** The load-bearing sentence: **a schedule prepares a transfer; it does not authorise one.** Every occurrence takes a fresh quote, runs screening again and is judged against the limits again, by calling the same `TransfersService.create` the send screen calls — no flag, no bypass, no second path into the ledger. It also does not pay: with no direct-debit mandate, an occurrence produces a transfer awaiting the sender's payment and tells them so, and the UI says that in those words. Corridors under exchange control **cannot be scheduled at all**, because a declaration is an affirmation about a specific payment at the moment it is made, and replaying one for a year is a signature forged on the sender's behalf.
- **14.5 Rate alerts.** Threshold alerts checked against **the rate we would actually quote**, margin included — alerting on the mid-market rate and then quoting something worse is a small dishonesty that costs a customer permanently. Fires once and disarms, because a rate hovering at a threshold would otherwise notify every minute and teach people to ignore notifications.
- **14.6 Support threads.** Customer correspondence, deliberately **not** a `ComplianceCase`. A compliance case is a regulated record with a decision, a reason and four-eyes; a support thread is a conversation about a late payment. Merging them would either drip customer chatter into the queue a regulator reads or strip the controls from regulated decisions. They meet in one place only: a thread about a transfer carries its id.

Two decisions recorded here because they will be asked again. **Held multi-currency balances were not built**: a persistent user balance is stored value, needing an e-money or PSSP authorisation and safeguarding, which is a CEO and counsel decision rather than a sprint. **Bill payment and airtime were not built**: they make us pay merchants, which is the commercial-payments business guardrail 2 exists to keep us out of.
