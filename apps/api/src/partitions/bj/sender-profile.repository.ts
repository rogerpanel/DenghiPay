...
/**
 * BJ partition — Beninese-resident sender personal data.
 *
 * UEMOA/BCEAO, with APDP oversight of the personal data.
 *
 * Reached only through `PartitionGateway`. Nothing outside `src/partitions/bj`
 * may import this file, and this file may not import from a sibling partition;
 * CI enforces both.
 */
@Injectable()
export class SenderProfileBjRepository {
  ...
  async upsert(input: {
    ...
    await this.prisma.senderProfileBj.upsert({
  ...
  async screeningProjection(
    ...
    const row = await this.prisma.senderProfileBj.findUnique({
      where: { piiToken },
      select: {
        firstName: true,
        middleName: true,
  ...
  async exists(piiToken: string): Promise<boolean> {
    return (await this.prisma.senderProfileBj.count({ where: { piiToken } })) > 0;
  }
}
